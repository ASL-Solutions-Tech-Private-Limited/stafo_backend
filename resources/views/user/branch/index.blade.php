        ...
        <div>
            <div class="row mb-3">
                ...
                <div class="col-md-3 col-6 text-end">
                    <a href="{{ route('branche.create') }}" class="btn btn-success shadow-sm"><i class="fas fa-plus"></i>
                        Create</a>
                </div>
            </div>
                    ...
                    <tbody>
                        @forelse ($branches as $index => $branch)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('branche.edit', $branch->id) }}" class="btn btn-warning btn-sm"><i
                                            class="fas fa-edit"></i></a>

                                    <!-- Delete Form with Confirmation -->
                                    <form id="delete-form-{{ $branch->id }}"
                                        action="{{ route('branche.destroy', $branch->id) }}" method="POST"
                                        style="display: inline;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="btn btn-danger btn-sm"
                                            onclick="confirmDelete(event, {{ $branch->id }})"><i
