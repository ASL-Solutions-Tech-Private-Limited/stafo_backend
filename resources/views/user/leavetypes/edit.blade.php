...
@section('title', 'Edit Leave Type | STAFO HRMS')
        ...
        <div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3 mb-4 pb-3 border-bottom">
            <div>
                <div class="d-flex align-items-center gap-2 mb-1">
                    <h3 class="fw-bold text-dark mb-0">Edit Leave Type</h3>
                    <span class="badge-stafo badge-stafo-primary">
                        <i class="fa-solid fa-plane-departure me-1"></i> {{ $leavetype->name }}
                    </span>
                </div>
                <p class="text-muted small mb-0">Update annual quotas, payment classification, and policy details</p>
            </div>
        ...
        <form action="{{ route('leavetypes.update', $leavetype->id) }}" method="POST">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-12 col-md-6">
                        ...
                        <input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror" 
                               value="{{ old('name', $leavetype->name) }}" placeholder="e.g. Annual Leave, Sick Leave" required>
                        @error('name')
                    ...
                    <div class="col-12 col-md-6">
                        ...
                        <input type="number" step="0.5" name="no_of_days" id="no_of_days" class="form-control @error('no_of_days') is-invalid @enderror" 
                               value="{{ old('no_of_days', $leavetype->no_of_days) }}" placeholder="e.g. 12" required>
                        @error('no_of_days')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-12 col-md-6">
                        ...
                        <select name="is_paid" id="is_paid" class="form-select @error('is_paid') is-invalid @enderror">
                            <option value="1" {{ old('is_paid', $leavetype->is_paid) == 1 ? 'selected' : '' }}>Paid Leave (Encashable/Covered)</option>
                            <option value="0" {{ old('is_paid', $leavetype->is_paid) == 0 ? 'selected' : '' }}>Unpaid Leave (LWP)</option>                            
                        </select>
                        @error('is_paid')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="description" id="description" class="form-control @error('description') is-invalid @enderror" 
                                  rows="3" placeholder="Provide details on eligibility, probation rules, or notice requirements">{{ old('description', $leavetype->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
            ...
            <div class="d-flex justify-content-end align-items-center gap-2 pt-3 border-top">
                ...
                <button type="submit" class="btn btn-primary px-5 py-2 fw-bold shadow-sm">
                    <i class="fa-solid fa-floppy-disk me-1"></i> Update Leave Type
                </button>
            </div>
        </form>
