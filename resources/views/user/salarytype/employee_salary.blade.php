...
@section('content')
    <div class="card mt-4 p-3 shadow-sm border-0">
        <form id=salary_export action="{{ route('salary-export') }}" method="GET">
            <div class="row mb-3">
                <div class="col-md-10"> &nbsp
                </div>
                <div class="col-md-2 text-right">
                    ...
                    <button type="button" id="export" class="btn btn-primary w-100"><i class="fas fa-download "></i> Export</button>
                </div>
            </div>
            </form>
                    ...
                    <tbody>
                        @forelse ($employeeSalaries as $index => $salary)
                            <tr>
                                ...
                                <td>
                                    <a
                                        href="{{ route('employeeSalaryDetails', $salary->employee_id) }}?month={{ $salary->salary_month }}&&year={{ $salary->salary_year }}"><i
                                            class="fas fa-eye"></i></a>
                                    {{-- <a
                                        href="{{ route('salaryPDF') }}?emp_id={{ $salary->employee_id }}&&month={{ $salary->salary_month }}&&year={{ $salary->salary_year }}"><i
                                            class="fas fa-file-pdf"></i></a> --}}
                                    <a
                                        href="{{ route('salaryPDF') }}?emp_id={{ $salary->employee_id }}&month={{ $salary->salary_month }}&year={{ $salary->salary_year }}">
