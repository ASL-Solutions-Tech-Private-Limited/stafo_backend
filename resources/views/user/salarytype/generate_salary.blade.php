...
@section('title', 'Payroll Generation Studio | STAFO HRMS')
...
<div class="card shadow-sm border-0 rounded-4">
    <div class="card-body p-4">
        ...
        <div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3 mb-4">
            <div>
                <div class="d-inline-flex align-items-center gap-2 px-3 py-1 rounded-pill bg-primary bg-opacity-10 text-primary fw-bold small mb-2">
                    <i class="fa-solid fa-coins"></i> Monthly Payroll Engine
                </div>
                <h3 class="fw-bold text-dark mb-1">Payroll Generation Studio</h3>
                <p class="text-muted small mb-0">Compute accurate attendance-linked compensation, statutory deductions, and issue verified payslips.</p>
            </div>
            <div class="d-flex align-items-center gap-2">
                <a href="{{ route('employeeSalaryList', ['month' => $currentMonth ?? date('m'), 'year' => $currentYear ?? date('Y')]) }}" class="btn btn-outline-secondary px-3 py-2 rounded-3">
        ...
        <form action="{{ route('saveEmployeeSalary') }}" method="POST" id="salaryForm">
            @csrf

            <div class="p-4 rounded-4 border mb-4" style="background: linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%);">
                <div class="row g-3 align-items-end">
                    <div class="col-12 col-md-3">
                        <label for="month" class="form-label fw-bold text-dark mb-1">
                        ...
                        <select class="form-select border-primary-subtle py-2" name="month" id="payroll_month" required>
                            ...
                            @endphp
                            <option value="1" {{ $selectedM == 1 ? 'selected' : '' }}>January</option>
                            <option value="2" {{ $selectedM == 2 ? 'selected' : '' }}>February</option>
                            <option value="3" {{ $selectedM == 3 ? 'selected' : '' }}>March</option>
                            <option value="4" {{ $selectedM == 4 ? 'selected' : '' }}>April</option>
                            <option value="5" {{ $selectedM == 5 ? 'selected' : '' }}>May</option>
                            <option value="6" {{ $selectedM == 6 ? 'selected' : '' }}>June</option>
                            <option value="7" {{ $selectedM == 7 ? 'selected' : '' }}>July</option>
                            <option value="8" {{ $selectedM == 8 ? 'selected' : '' }}>August</option>
                            <option value="9" {{ $selectedM == 9 ? 'selected' : '' }}>September</option>
                            <option value="10" {{ $selectedM == 10 ? 'selected' : '' }}>October</option>
                            <option value="11" {{ $selectedM == 11 ? 'selected' : '' }}>November</option>
                            <option value="12" {{ $selectedM == 12 ? 'selected' : '' }}>December</option>
                        </select>
                    </div>

                    <div class="col-12 col-md-3">
                        <label for="year" class="form-label fw-bold text-dark mb-1">
