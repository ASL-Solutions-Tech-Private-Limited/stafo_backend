...
@section('title', 'Edit Comp-Off Leave | STAFO HRMS')
        ...
        <div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3 mb-4 pb-3 border-bottom">
            <div>
                <div class="d-flex align-items-center gap-2 mb-1">
                    <h3 class="fw-bold text-dark mb-0">Edit Comp-Off Leave</h3>
                    <span class="badge-stafo badge-stafo-primary">
                        <i class="fa-solid fa-business-time me-1"></i> {{ $compoffleave->title }}
                    </span>
                </div>
                <p class="text-muted small mb-0">Modify compensatory off request details and approval status</p>
            </div>
        ...
        <form action="{{ route('compoffleaves.update', $compoffleave->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row g-3">
                        ...
                        <select class="form-select @error('employee_id') is-invalid @enderror" name="employee_id" id="employee_id" required>
                            ...
                            @if (!$employees->isEmpty())
                                @foreach ($employees as $employee)
                                    <option value="{{ $employee->id }}" {{ old('employee_id', $compoffleave->employee_id) == $employee->id ? 'selected' : '' }}>
                                        {{ $employee->name }}
                                    </option>
                                @endforeach
                            @endif
                    ...
                    <div class="col-12 col-md-6">
                        ...
                        <input type="text" name="title" id="title" class="form-control @error('title') is-invalid @enderror" 
                               value="{{ old('title', $compoffleave->title) }}" placeholder="e.g. Weekend work, Holiday shift coverage" required>
                        @error('title')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-12 col-md-6">
                        ...
                        <input type="date" name="date" id="date" class="form-control @error('date') is-invalid @enderror" 
                               value="{{ old('date', $compoffleave->date) }}" required>
                        @error('date')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-12 col-md-6">
                        ...
                        <textarea name="description" id="description" class="form-control @error('description') is-invalid @enderror" 
                                  rows="3" placeholder="Reason for granting compensatory leave">{{ old('description', $compoffleave->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                            ...
                            <div class="mt-2 p-2 bg-white rounded-3 border d-flex align-items-center justify-content-between">
                                <div class="d-flex align-items-center gap-2">
                                    ...
                                    <div>
                                        ...
                                        <span class="small fw-semibold text-dark">{{ $compoffleave->filename }}</span>
                                    </div>
                                </div>
                                <a href="{{ asset('uploads/compoff/' . $compoffleave->filename) }}" target="_blank" 
            ...
            <div class="d-flex justify-content-end align-items-center gap-2 pt-3 border-top">
                ...
                <button type="submit" class="btn btn-primary px-5 py-2 fw-bold shadow-sm">
                    <i class="fa-solid fa-floppy-disk me-1"></i> Update Comp-Off Leave
                </button>
            </div>
        </form>
