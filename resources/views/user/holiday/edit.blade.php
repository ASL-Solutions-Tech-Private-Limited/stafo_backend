...
@section('title', 'Edit Company Holiday | STAFO HRMS')
...
<style>
    /* ========================================================
       Edit Holiday Form - STAFO Modern Design
       ======================================================== */
    .holiday-form-hero {
            ...
            <div>
                <div class="d-flex align-items-center gap-2 mb-1">
                    <span class="badge rounded-pill px-3 py-1 fw-bold" style="background: rgba(255,255,255,0.2); color: #ffffff; font-size: 0.74rem;">
                        <i class="fa-solid fa-pen-to-square me-1 text-warning"></i> Edit Holiday
                    </span>
                    <span class="text-white-50 small">• ID #{{ $holiday->id }}</span>
                </div>
                <h3 class="fw-bold text-white mb-1">Edit Company Holiday</h3>
                <p class="text-white small mb-0 opacity-90">
                    Update holiday details, title, and calendar schedule for <strong>{{ $holiday->title }}</strong>.
                </p>
            </div>
            <div class="d-flex align-items-center gap-2">
                <a href="{{ route('holiday.index') }}" class="btn btn-outline-light btn-sm rounded-3 py-2 px-3 fw-semibold">
                    <i class="fa-solid fa-arrow-left me-1"></i> Back to Holidays
                </a>
            </div>
        </div>
    </div>
            ...
            <div class="form-card-container">
                <div class="form-section-header">
                    <h6 class="fw-bold mb-0 text-dark d-flex align-items-center gap-2">
                        ...
                        <span>Modify Holiday Information</span>
                    </h6>
                    <span class="text-muted small">All marked (*) fields are required</span>
                </div>

                <div class="p-4">
                    <form action="{{ route('holiday.update', $holiday->id) }}" method="POST" id="holidayEditForm">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <div class="input-group holiday-input-group">
                                <span class="input-group-text"><i id="titleInputIcon" class="fa-solid fa-umbrella-beach text-primary"></i></span>
                                <input type="text" name="title" id="title" class="form-control form-control-lg" 
                                       value="{{ old('title', $holiday->title) }}" placeholder="e.g. New Year's Day, Diwali, Independence Day, Christmas" 
                            ...
                            <div class="mt-2.5">
                                <small class="text-muted fw-semibold d-block mb-1.5" style="font-size: 0.72rem; letter-spacing: 0.3px; text-transform: uppercase;">
                                    <i class="fa-solid fa-wand-magic-sparkles text-warning me-1"></i> Quick Festival Suggestions:
                                </small>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group holiday-input-group">
                                    ...
                                    <input type="date" name="start_date" id="start_date" class="form-control" 
                                           value="{{ old('start_date', $holiday->start_date) }}" required onchange="onStartDateChange()">
                                </div>
                                @error('start_date')
                                    <div class="text-danger small mt-1"><i class="fa-solid fa-circle-exclamation me-1"></i>{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group holiday-input-group">
                                    ...
                                    <input type="date" name="end_date" id="end_date" class="form-control" 
                                           value="{{ old('end_date', $holiday->end_date ?? $holiday->start_date) }}" required onchange="updateLivePreview()">
                                </div>
                                @error('end_date')
                                    <div class="text-danger small mt-1"><i class="fa-solid fa-circle-exclamation me-1"></i>{{ $message }}</div>
                                @enderror
                        ...
                        <div class="mb-4">
                            ...
                            <div class="input-group holiday-input-group">
                                ...
                                <textarea name="description" id="description" class="form-control" rows="3" 
                                          ...
                                          oninput="updateLivePreview()">{{ old('description', $holiday->description) }}</textarea>
                            </div>
                        </div>
            ...
            <div class="form-card-container mb-4">
                ...
                <div class="p-3">
                    <small class="text-muted d-block mb-3" style="font-size: 0.75rem;">
                        Live preview of how this holiday appears in the company & employee calendars:
                    </small>
                    ...
                    <div class="holiday-preview-box">
                        <div class="d-flex align-items-center gap-2.5 mb-2.5">
                            <div id="prevIconBox" class="rounded-3 d-flex align-items-center justify-content-center text-white shadow-sm fs-4" 
                                 style="width: 48px; height: 48px; flex-shrink: 0; background: linear-gradient(135deg, #10b981 0%, #059669 100%);">
                                <i id="prevIcon" class="fa-solid fa-umbrella-beach"></i>
                            </div>
                            <div>
                                <h6 class="fw-bold text-dark mb-0 d-flex align-items-center gap-1.5">
                                    <span id="prevEmoji">🏖️</span>
                                    <span id="prevTitle">{{ $holiday->title }}</span>
                                </h6>
                                <div class="d-flex align-items-center gap-1 mt-1">
                                    <span class="badge py-0.5 px-1.5" id="prevCategoryBadge" style="background: rgba(16, 185, 129, 0.15); color: #065f46; font-size: 0.65rem;">Company Holiday</span>
                        ...
                        <div>
                            ...
                            <p class="text-muted small mb-0 lh-sm" id="prevDesc" style="font-size: 0.8rem;">
                                {{ $holiday->description ?? 'Official company holiday announced by management.' }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="mt-3 p-3 rounded-3 bg-light border">
                        ...
                        <div class="holiday-chip d-inline-flex" id="prevChip" style="font-size: 0.78rem; padding: 5px 10px;">
                            <i id="prevChipIcon" class="fa-solid fa-umbrella-beach me-1"></i> <span id="prevChipText">{{ $holiday->title }}</span>
                        </div>
                    </div>
                </div>
            </div>
