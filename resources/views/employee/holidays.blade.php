...
@section('title', 'Holiday Calendar | STAFO HRMS')

@section('css')
<style>
    /* ========================================================
       STAFO Holiday Calendar Styles (Light & Dark Theme)
       ======================================================== */
    .holiday-hero-card {
    ...
    /* ========================================================
       ...
       ======================================================== */
    [data-theme="dark"] .holiday-hero-card {
...
<div class="container-fluid p-0">
    ...
    @php
        ...
        $upcomingHolidaysList = $holidays->filter(function($h) use ($currentDate) {
            return Carbon\Carbon::parse($h->start_date)->gte($currentDate->copy()->startOfDay());
        });
        $nextHoliday = $upcomingHolidaysList->first();
        $totalHolidaysCount = $holidays->count();
        $upcomingCount = $upcomingHolidaysList->count();
    @endphp

    <!-- Top Banner & Next Holiday Showcase -->
            ...
            <div class="col-lg-7">
                <div class="d-flex align-items-center gap-2 mb-2">
                    <span class="badge rounded-pill px-3 py-1.5 fw-bold" style="background: rgba(255,255,255,0.2); color: #ffffff; font-size: 0.76rem; border: 1px solid rgba(255,255,255,0.3);">
                        <i class="fa-solid fa-umbrella-beach me-1 text-warning"></i> Company Holidays {{ date('Y') }}
                    </span>
                    <span class="text-white-50 small">• Official Leave Observances</span>
                </div>
                <h3 class="fw-bold text-white mb-2">
                    Holiday Calendar & Observances
                </h3>
                <p class="text-white small mb-0 opacity-90" style="max-width: 580px;">
                    Plan your vacations, personal time, and work-life balance around officially recognized company holidays.
                </p>
            ...
            <div class="col-lg-5 text-lg-end">
                <div class="d-inline-flex flex-wrap align-items-center justify-content-lg-end gap-2">
                    <div class="holiday-stat-box text-center">
                        ...
                        <h4 class="fw-bold text-white mb-0">{{ $totalHolidaysCount }}</h4>
                    </div>
                    <div class="holiday-stat-box text-center">
    ...
    <!-- Festive Upcoming Holidays Marquee Ticker -->
    @php
        $futureHolidays = $holidays->filter(function($h) {
            return \Carbon\Carbon::parse($h->start_date)->endOfDay()->isFuture();
        })->sortBy('start_date');
    @endphp
    ...
    <div class="card border-0 shadow-sm rounded-4 mb-4 emp-card">
        <div class="card-header bg-white border-bottom p-3 p-md-4 d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3 emp-card-header">
            <!-- Left: Title & Selected Month Header -->
            ...
            <!-- Right: View Mode Toggle & Navigation Controls -->
            <div class="d-flex flex-wrap align-items-center gap-2">
                ...
                <a href="{{ route('employee.dashboard') }}" class="btn btn-outline-secondary btn-sm rounded-3 py-1.5 px-3 fw-semibold">
        ...
        <div id="calendarViewContainer">
            ...
            <div class="p-3 border-top d-flex flex-wrap align-items-center justify-content-between gap-3 text-muted small bg-light bg-opacity-25">
                ...
                <span class="text-muted small">Tip: Click on any holiday chip to view details</span>
            </div>
        </div>
        ...
        <div id="listViewContainer" style="display: none;">
            <div class="p-3 p-md-4 border-bottom d-flex flex-column flex-sm-row align-items-sm-center justify-content-between gap-2">
                <span class="text-muted small fw-semibold">Showing all official holidays registered for this organization</span>
                    ...
                    <tbody>
                        @forelse($holidays as $index => $holiday)
                            ...
                            <tr class="holiday-list-row {{ $isUpcoming ? 'table-success bg-opacity-10' : '' }}">
                                    ...
                                    <div class="d-flex align-items-center gap-2.5">
                                        ...
                                        <div>
                                            ...
                                            <div class="d-flex align-items-center gap-1 mt-0.5">
                                                ...
                                                @if($durationDays > 1)
                                                    <span class="badge bg-info text-info py-0.5 px-1.5" style="font-size: 0.65rem;">{{ $durationDays }} Days Holiday</span>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <td>
                                    <span class="text-muted small fw-semibold">{{ $sDate->format('l') }}</span>
                                </td>
                                <td class="small text-muted">
                                    @if($eDate && $eDate->gt($sDate))
                                        {{ $eDate->format('d M, Y') }} ({{ $durationDays }} days)
                                    @else
                                        1 Day
                                    @endif
                                </td>
                                ...
                                <td>
                                    @if($sDate->isToday())
                                        <span class="badge bg-success text-success">Today!</span>
                                    @elseif($isUpcoming)
                                        <span class="badge bg-success text-success">
                                            In {{ $daysDiff }} {{ $daysDiff === 1 ? 'Day' : 'Days' }}
                                        </span>
                                    @else
                                        <span class="badge bg-secondary text-secondary">Past Holiday</span>
                                    @endif
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="7" class="text-center py-5 text-muted">
                                    ...
                                    No holidays declared for your organization yet.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
...
<!-- Interactive Holiday Details Modal -->
<div class="modal fade" id="holidayDetailModal" tabindex="-1" aria-labelledby="holidayModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
            <div class="modal-header py-3" style="background: linear-gradient(135deg, #064e3b 0%, #059669 100%); color: #ffffff;">
                <h6 class="modal-title fw-bold d-flex align-items-center gap-2" id="holidayModalLabel">
            ...
            <div class="modal-body p-4">
                ...
                <div class="row g-2 mb-3">
                    <div class="col-6">
                        <div class="p-2.5 rounded-3 bg-light border text-center">
                            <small class="text-muted d-block text-uppercase fw-bold" style="font-size: 0.68rem;">Date</small>
                            <span class="fw-bold text-dark" id="modalHolidayDate">--</span>
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="p-2.5 rounded-3 bg-light border text-center">
                            <small class="text-muted d-block text-uppercase fw-bold" style="font-size: 0.68rem;">Day of Week</small>
                            <span class="fw-bold text-primary" id="modalHolidayDay">--</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="d-grid">
                    <button type="button" class="btn btn-secondary rounded-3 py-2 fw-semibold" data-bs-dismiss="modal">
                        Close
                    </button>
                </div>
            </div>
...
@section('js')
<script>
    // 1. Parse Holidays from Backend
    const holidaysData = @json($holidays);

    // Map holidays into quick key-value lookup: 'YYYY-MM-DD' => array of holidays
    const holidaysMap = {};
    ...
    // 2. Calendar State
    let viewDate = new Date(); // Currently viewed year & month
    ...
    // 3. Render Quick Month Navigation Bar
    ...
    function renderCalendar() {
        ...
        // 3. Next Month Spillover Days (to complete 35 or 42 grid cells)
    ...
    // Helper: Create individual day cell element
    function createDayCell(dateObj, isOtherMonth) {
        ...
        dayHolidays.forEach(h => {
            ...
            chip.title = `${fest.emoji ? fest.emoji + ' ' : ''}${h.title} (${fest.category}) - Click to view`;
            chip.onclick = (e) => {
                e.stopPropagation();
        ...
        // Click cell to view holiday if exists
    ...
    // 6. View Mode Switching (Calendar vs List)
    function switchViewMode(mode) {
        ...
        if (mode === 'calendar') {
            ...
            if (btnCal) {
                btnCal.className = 'btn active';
            }
            if (btnList) {
                btnList.className = 'btn text-muted';
            }
        } else {
            if (calContainer) calContainer.style.display = 'none';
            if (listContainer) listContainer.style.display = 'block';
            if (btnCal) {
                btnCal.className = 'btn text-muted';
            }
            if (btnList) {
                btnList.className = 'btn active';
            }
        }
    }
    ...
    // 7. Interactive Holiday Details Modal
    function openHolidayDetailModal(holiday) {
        ...
        document.getElementById('modalHolidayDate').textContent = sDate.toLocaleDateString('en-US', options);
        document.getElementById('modalHolidayDay').textContent = sDate.toLocaleDateString('en-US', { weekday: 'long' });
    ...
    // 8. Filter List Table by Search Term
