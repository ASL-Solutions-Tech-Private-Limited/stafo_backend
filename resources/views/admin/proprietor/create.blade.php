        ...
        <div class="app-main__inner">
            <div class="d-flex justify-content-between user-access">
                <div class="user-welcome">
                    <h3>Add New Proprietor</h3>
                </div>
                <!-- <button class="btn primary-bg"><i class="fa-solid fa-plus"></i> Add New User</button> -->
            </div>
            <form action="{{ route('proprietor.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="mobile_no" id="mobile" class="form-control"
                                value="{{ old('mobile_no') }}" placeholder="Enter Mobile">
                            @error('mobile_no')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="aadhar" id="aadhar" class="form-control"
                                value="{{ old('aadhar') }}" placeholder="Enter Aadhar">
                            @error('aadhar')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="pan" id="pan" class="form-control"
                                value="{{ old('pan') }}" placeholder="Enter PAN">
                            @error('pan')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                            ...
                            <select name="current_country" id="current_country" class="form-control">
                                ...
                                @foreach ($countries as $country)
                                    <option value="{{ $country->id }}"
                                        {{ old('current_country') == $country->id ? 'selected' : '' }}>
                                        {{ $country->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-3 mb-3">
                            ...
                            <select name="current_state" id="current_state" class="form-control" disabled>
                        ...
                        <div class="col-md-3 mb-3">
                            ...
                            <select name="current_city" id="current_city" class="form-control" disabled>
                        ...
                        <div class="col-md-12 mb-3">
                            <input type="checkbox" id="same_as_current" name="same_as_current" onclick="copyAddress()">
                            <label for="same_as_current" class="text-dark">Check if Permanent Address is the same as
                                Current
                                Address</label>
                        </div>
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            <textarea name="p_address" id="p_address" class="form-control" rows="2"
                                placeholder="Enter Permanent Address">{{ old('p_address') }}</textarea>
                            @error('p_address')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select name="p_country" id="p_country" class="form-control">
                                ...
                                @foreach ($countries as $country)
                                    <option value="{{ $country->id }}"
                                        {{ old('p_country') == $country->id ? 'selected' : '' }}>
                                        {{ $country->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select name="p_city" id="p_city" class="form-control">
                                <option value="">Select City</option>
                                <option value="29">KURUNG KUMEY</option>
                                @foreach ($cities as $city)
                                    <option value="{{ $city->id }}"
                                        {{ old('p_city') == $city->id ? 'selected' : '' }}>
                                        {{ $city->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="p_pin" id="p_pin" class="form-control"
                                value="{{ old('p_pin') }}" placeholder="Enter Permanent Pin">
                            @error('p_pin')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
...
@section('scripts')
    <script>
        function copyAddress() {
            const sameAsCurrent = document.getElementById('same_as_current').checked;
            ...
            document.getElementById('p_address').value = sameAsCurrent ? currentAddress : '';
            document.getElementById('p_country').value = sameAsCurrent ? currentCountry : '';
            document.getElementById('p_state').value = sameAsCurrent ? currentState : '';
            document.getElementById('p_city').value = sameAsCurrent ? currentCity : '';
            document.getElementById('p_pin').value = sameAsCurrent ? currentPin : '';
        }
    </script>
