    ...
    <div class="card mt-4 p-3 shadow-sm border-0">
        ...
        <div class="row mb-3">
            ...
            <div class="col-md-3 col-3 text-end">
                <a href="{{ route('admin.expenseCreate') }}?company_id={{$companyId}}" class="btn btn-success shadow-sm"><i class="fas fa-plus"></i>
                    Create</a>
            </div>
        </div>
                ...
                <tbody>
                    ...
                    @foreach ($expensees as $index => $expense)
                        <tr>
                            ...
                            <!-- Actions -->
                            <td>
                                <a href="{{ route('admin.expenseDetails', $expense->id) }}?company_id={{$companyId}}" class="btn btn-info btn-sm"><i
                                        class="fas fa-eye"></i></a>
                                <a href="{{ route('admin.expenseEdit', $expense->id) }}?company_id={{$companyId}}" class="btn btn-warning btn-sm"><i
                                        class="fas fa-edit"></i></a>
                                <form action="{{ route('admin.expenseDelete', $expense->id) }}?company_id={{$companyId}}" method="POST"
    ...
    <script>
        ...
        $(document).ready(function() {
            $('.status_change').on('click', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: `{{ route('admin.expensestatuschange') }}`,
                            type: 'POST',
                            data: {
                                _token: '{{ csrf_token() }}',
                                status: status,
            ...
            $('#company_name').on('change', function () {               
                const id = $(this).val();
                const url = '{{ route('admin.expenseList') }}'+'?company_id=' + id;
