...
@section('content')
    ...
    <div class="card mt-4 p-3 ">
        <div class="container">
            ...
            <form action="{{ route('admin.taskUpdate',$task->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('POST')
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group mb-3">
                            <label for="task_assign ">Company</label>
                            <select name="company_id" class="form-control tomselect2" required>
                                @if(count($companies) > 0)
                                    <option value="">Select Company</option>
                                    @foreach ($companies as $company)
                                        <option value="{{ $company->id }}" @if($task->company_id == $company->id) selected @endif>
        ...
        function confirmDelete(event, fileId) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: '{{ url('/') }}' + '/admin/task/file-delete/' + fileId,
                        type: 'DELETE',
                        dataType: 'json',
                        data: {'_token': token},
